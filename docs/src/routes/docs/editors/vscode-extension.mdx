...
Priority is applied per config field:

1. value from the nearest project config file (`tombi.toml`, `.tombi.toml`, `pyproject.toml`)
2. value from `tombi.config` in VSCode settings
